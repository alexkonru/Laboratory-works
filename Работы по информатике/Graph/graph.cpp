...
Graph::Graph(QWidget *parent) : QGraphicsView(parent){
    ...
    scene->setSceneRect(-400, -450, 1915, 1075);
    setScene(scene);
...
void Graph::createTabWidget(const QRectF &rect){
    ...
    setVertexPosButton->setText(tr("Позиция"));
    ...
    exit->setMaximumSize(QSize(120, 35));
    exit->setStyleSheet("QPushButton:selected, QPushButton:hover{color: darkBlue;} "
                                    "QPushButton:pressed{background-color: lightBlue;} "
                                    "QPushButton{height: 30px; width: 30px; background-color: lightGray; border-right, border-bottom: 4px solid Gray;}");

    connect(exit, &QPushButton::clicked, this, &Graph::exitWindow);
    ...
    vertexTabLayout->addWidget(exit);
    ...
    QAction *dfs = new QAction(tr("Поиск в глубину"), this);
    dfs->setStatusTip(tr("Поиск в глубину"));
    connect(dfs, &QAction::triggered, this, &Graph::createDFSWindow);
    menu->addAction(dfs);
    menu->addSeparator();

    QAction *bfs = new QAction(tr("Поиск в ширину"), this);
    bfs->setStatusTip(tr("Поиск в ширину"));
    connect(bfs, &QAction::triggered, this, &Graph::createBFSWindow);
    menu->addAction(bfs);
    menu->addSeparator();

    QAction *dijkstra = new QAction(tr("Алгоритм Дейкстры"), this);
    bfs->setStatusTip(tr("Алгоритм Дейкстры"));
...
void Graph::getVertexInfo(int vertexIndex){
    ...
    infoWindow->setItem(3, 0, new QTableWidgetItem("Посещена"));
    bool visited = false;
...
void Graph::getGraphInfo(){
    ...
    infoWindow->setItem(0, 0, new QTableWidgetItem("Индекс вершины"));
    infoWindow->setItem(0, 1, new QTableWidgetItem(QString::number(vertexNum)));
    infoWindow->setItem(1, 0, new QTableWidgetItem("Индекс ребра"));
    infoWindow->setItem(1, 1, new QTableWidgetItem(QString::number(edgeNum)));
    infoWindow->setItem(2, 0, new QTableWidgetItem("Связанный граф"));
    ...
    QString dfs = "Поиск в глубину с индекса  ";
    if(!DFS.empty()){
    ...
    QString bfs = "Поиск в ширину с индекса  ";
    if(!BFS.empty()){
...
void Graph::createEraseVertexWindow(){
    ...
    layout->addWidget(new QLabel(tr("Удалить вершину:")), 0, 0);
...
void Graph::createSetVertexPosWindow(){
    ...
    layout->addWidget(input1, 0, 1);
    layout->addWidget(new QLabel(tr("X координата")), 1, 0);
    layout->addWidget(input2, 1, 1);
    layout->addWidget(new QLabel(tr("Y координата:")), 2, 0);
    layout->addWidget(input3, 2, 1);
    layout->addWidget(okButton, 3, 1, Qt::AlignRight);
    layout->setSizeConstraint(QLayout::SetFixedSize);
    window->setLayout(layout);
    window->setWindowTitle("Положение вершины");
...
void Graph::createEraseEdgeWindow(){
    ...
    layout->setSizeConstraint(QLayout::SetFixedSize);
    window->setLayout(layout);
    window->setWindowTitle("Удалить ребро");
...
void Graph::createUpdateWeightWindow(){
    ...
    layout->setSizeConstraint(QLayout::SetFixedSize);
    window->setLayout(layout);
    window->setWindowTitle("Изменить вес ребра");
