...
LinkedList::LinkedList(int s) {
    if (s > max_size) throw error("List length more than max_size\n");
    size = s;
    for (int i = 0; i < s; i++) {
        push_back(0);
    }
...
{
    if (size + 1 == max_size) throw error("Size > max_size\n");
    push_front(a);
    return *this;
}
LinkedList LinkedList::operator+(LinkedList& l)
{
    if (head == nullptr && l.head == nullptr) throw error("Lists are empty\n");
